# OGify

OGify – Dynamic Open Graph Image Generator creates 1200x630 PNG share images for WordPress posts and outputs matching Open Graph and Twitter Card meta tags.

## Install

Copy this folder to `wp-content/plugins/ogify`, then activate **OGify – Dynamic Open Graph Image Generator** in WordPress.

## Settings

Open **Settings > OGify** to configure post types, author photo/name visibility, reading time, site name text, background colors, text color, accent color, reading speed, and a default author photo. Per-user OGify share photos live on each user profile screen.

Generated images are cached in `uploads/ogify` and refresh when post content or card settings change.

## License

GPLv3. See `assets/fonts/OFL.txt` for the bundled Inter font license.
